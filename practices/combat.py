...
import random
import time
fighter_stats = {"Class":"Fighter","Speed":9,"Offense":15,"Defense":13,"Health":100,"Attack":"D20"}
rouge_stats = {"Class":"Rouge","Speed":15,"Offense":12,"Defense":12,"Health":80,"Attack":"D20"}
mage_stats = {"Class":"Mage","Speed":12,"Offense":21,"Defense":11,"Health":60,"Attack":"D20"}
admin_stats = {"Class":"God","Speed":1000,"Offense":1000,"Defense":1000,"Health":1000,"Attack":"D20"}
...
def win_condition(player_health,monster_health):
    if player_health <= 0:
...
while True:
    print("Classes: \n1 Fighter: A melee focused attacker.\n2 Rouge: A sneaky, speedy little gremlin.\n3 Mage: A magic focused glass cannon.\n4 Admin: Used for playtesting the game by the dev, has bonkers stats.")
    while True:
        player_class = input("Pick your class! 1/2/3/4").strip()
        match player_class:
            case "1":
            ...
            case "3":
                ...
                break
            case "4":
                player_class = admin_stats
                player_health = admin_stats["Health"]
                break
            case _:
    ...
    while True:
        if lazy_fix == False:
            if turn == 1:
                ...
                if player_action == "1":
                    player_damage = player_attack(monster_type,player_class)
                    monster_health -= player_damage
                    print(f"The {monster_type["Type"]} now has {monster_health} health left.")
                    winner = win_condition(player_health,monster_health)
                    if winner == 1 or winner == 2:
                        break
                    else:
                        turn = 2
                elif player_action == "2":
                    player_damage = player_attack(monster_type,player_class) * 2
                    monster_health -= player_damage
                    player_health -= 6
                    print(f"The {monster_type["Type"]} now has {monster_health} health left.")
                    print(f"You took 6 damage.")
            ...
            if turn == 2:
                ...
                player_health -= monster_attack(player_class,monster_type)
                print(f"Player Health: {player_health}")
                winner = win_condition(player_health,monster_health)
                if winner == 1 or winner == 2:
                    break
